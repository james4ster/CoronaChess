...
async function run() {
  ...
  for (const season of allSeasons) {
    ...
    // 🔹 Get all week start dates for this season
    const allWeekStartDates = await getAllScheduledWeekStartDates(currentSeasonId); 
    const today = new Date();

    // 🔹 Only process weeks that have started (<= today)
    const weeksToProcess = allWeekStartDates.filter(
      d => new Date(d) <= today
    );

    for (const weekStartDate of weeksToProcess) {
      const scheduleForWeek = await getMatchupsForWeek(currentSeasonId, weekStartDate);
        ...
        try {
          ...
          for (const m of monthsToFetch) {
            ...
            const filtered = games.filter(g => {
              ...
              const isTenMinute = g.time_control === '600'; // 10-min games only

              return white && black && scheduledUsernames.has(white) && scheduledUsernames.has(black) && rulesMatch && isTenMinute;
            });
      ...
      const outputPath = path.join(
        dataDir,
        `games_${currentSeasonId}_${currentGameType}_${weekStartDate}.json`
      );
      ...
      for (const game of allGames) {
        ...
        const key = `${matchupKey}|${new Date(weekStartDate).toISOString().slice(0, 10)}`;
        const existing = matchupToFirstGame.get(key);

        if (earlyFlag) {
          if (gameDate >= new Date(weekStartDate)) continue;
      ...
      const resultsRows = newGames.map(game => {
        const row = transformGameToResultsRow(game, currentSeasonId, weekStartDate, currentGameType);
      ...
      if (resultsRows.length > 0) {
        ...
        console.log(`No new games to append for SeasonID ${currentSeasonId}, GameType ${currentGameType}, Week Start ${weekStartDate}.`);
      }

      notifyCommissioner(`Results updated for Season ${currentSeasonId} (${currentGameType}) Week ${weekStartDate} with ${resultsRows.length} new games.`);
    }
  }
}
